...
int morphological(unsigned char (&input_image)[h][w][c], int image_height, int image_width, string type){
    ...
    int count = 0;
    // construct the mask
    unsigned char mask[247][247][1];

    // step 2, pad the input image
    // construct padded image
    unsigned char padded_image[249][249][1];
    ...
    // step 5, pad the mask
    // padded mask
    unsigned char padded_mask[249][249][1];
...
int main(int argc, char *argv[]){
    ...
    unsigned char flower_data[247][247][1];

    // read in flower
    read_in(flower_data, argv[1], 247, 247, 1);

    // binarize 
    for (int i = 0; i < 247; i++){
        for (int j = 0; j < 247; j++){
            if (flower_data[i][j][0] >= 0x7F){
                flower_data[i][j][0] = 0xFF;
            }
            else{
                flower_data[i][j][0] = 0x00;
            }
        }
    }

    // zero pad for initial run
    unsigned char flower_padded[249][249][1];
    zero_pad(flower_padded, flower_data, 1, 247, 247);

    int iteration = 1;

    // Thinning operation
    if (strcmp(argv[3], "thinning")==0){
        cout << "performing thinning on flower..." << endl;
        string T = "T";
        int count = morphological(flower_data, 247, 247, T);
        cout << count << " ";

        while(count != 0){
            cout << count << " ";
            count = morphological(flower_data, 247, 247, T);
            iteration += 1;
        }
        cout << endl;
        cout << "total iterations: " << iteration << endl;
        write_out(flower_data, argv[2], 247, 247, 1);
    }

    else if (strcmp(argv[3], "shrinking")==0){
        cout << "performing shrinking on flower..." << endl;
        string T = "S";
        int count = morphological(flower_data, 247, 247, T);
        cout << count << " ";
        ...
        cout << "performing skeletonizing on flower..." << endl;
        string T = "K";
        int count = morphological(flower_data, 247, 247, T);
        cout << count << " ";
