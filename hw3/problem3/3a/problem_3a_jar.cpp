    ...
    for (int i = 0; i <= 251; i++){
        for (int j = 0; j <= 251; j++){
            ...
            if (mask[i][j][0] == 0xFF){
                ...
                // use the pattern to check unconditional
...
int main(int argc, char *argv[]){
    ...
    unsigned char jar_data[252][252][1];

    // read in flower
    read_in(jar_data, argv[1], 252, 252, 1);
    // binarize 
    for (int i = 0; i < 252; i++){
        for (int j = 0; j < 252; j++){
            if (jar_data[i][j][0] >= 127){
                jar_data[i][j][0] = 0xFF;
            }
            else{
                jar_data[i][j][0] = 0x00;
    ...
    unsigned char jar_padded[254][254][1];
    zero_pad(jar_padded, jar_data, 1, 252, 252);
    
    int iteration = 1;

    // Thinning operation
    if (strcmp(argv[3], "thinning")==0){
        cout << "performing thinning on jar..." << endl;
        string T = "T";
        int count = morphological(jar_data, 252, 252, T);
        cout << count << " ";

        while(count != 0){
            cout << count << " ";
            count = morphological(jar_data, 252, 252, T);
            iteration += 1;
        }
        cout << endl;
        cout << "total iterations: " << iteration << endl;
        write_out(jar_data, argv[2], 252, 252, 1);
    }

    else if (strcmp(argv[3], "shrinking")==0){
        cout << "performing shrinking on jar..." << endl;
        string T = "S";
        int count = morphological(jar_data, 252, 252, T);
        cout << count << " ";
        ...
        cout << "performing skeletonizing on jar..." << endl;
        string T = "K";
        int count = morphological(jar_data, 252, 252, T);
        cout << count << " ";
