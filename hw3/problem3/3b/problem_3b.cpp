/*
    ...
    cpp file for problem 3b (Solution to the maze)
*/
...
int morphological(unsigned char (&input_image)[h][w][c], int image_height, int image_width, string type){
    ...
    int count = 0;
    // construct the mask
    unsigned char mask[343][343][1];

    // step 2, pad the input image
    // construct padded image
    unsigned char padded_image[345][345][1];
    ...
    // step 5, pad the mask
    // padded mask
    unsigned char padded_mask[345][345][1];
...
int main(int argc, char *argv[]){
    ...
    unsigned char maze_data[343][343][1];

    // read in flower
    read_in(maze_data, argv[1], 343, 343, 1);

    // binarize 
    for (int i = 0; i < 343; i++){
        for (int j = 0; j < 343; j++){
            if (maze_data[i][j][0] >= 0x7F){
                maze_data[i][j][0] = 0xFF;
            }
            else{
                maze_data[i][j][0] = 0x00;
            }
        }
    }

    // fix the maze by setting the final row and column to 0
    for (int i = 0; i < 343; i++){
        maze_data[i][342][0] = 0xFF;
        maze_data[342][i][0] = 0xFF;
    }

    // iteration holder for intermediate result
    int iteration = 1;

    // perform shrinking on the maze
    cout << "performing shrinking on the maze..." << endl;
    string T = "S";
    int count = morphological(maze_data, 343, 343, T);
    cout << count << " ";

    while (count != 0){
        count = morphological(maze_data, 343, 343, T);
        cout << count << " ";
        iteration += 1;
        if (iteration == 170){
            char * path = (char *)"maze_1.raw";
            write_out(maze_data, path, 343, 343, 1);
        }
        if (iteration == 340){
            char * path = (char *)"maze_2.raw";
            write_out(maze_data, path, 343, 343, 1);   
        }
        if (iteration == 510){
            char * path = (char *)"maze_3.raw";
            write_out(maze_data, path, 343, 343, 1);  
        }
    }
    cout << endl;
    cout << "total number of iterations: " << iteration << endl;
    write_out(maze_data, argv[2], 343, 343, 1);
}